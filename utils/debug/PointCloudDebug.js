...
function setupControllerVisibily(gui, displayMode, sizeMode) {
    ...
    if (sizeMode === PNTS_SIZE_MODE.VALUE) {
        ...
        showController(gui, 'pointSize');
    } else {
        hideController(gui, 'pointSize');
...
export default {
    initTools(view, layer, datUi) {
        ...
        if (layer.material.sizeMode != undefined) {
            ...
            styleUI.add(layer.material, 'minAttenuatedSize', 0, 15).name('Min attenuated size')
                .onChange((value) => {
            ...
            styleUI.add(layer.material, 'maxAttenuatedSize', 0, 15).name('Max attenuated size')
                .onChange((value) => {
