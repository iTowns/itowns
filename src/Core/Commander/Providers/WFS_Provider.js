...
WFS_Provider.prototype.tileInsideLimit = function tileInsideLimit(tile, layer) {
    const bbox = new BoundingBox(layer.projection, layer.bbox[0], layer.bbox[2], layer.bbox[1], layer.bbox[3], 0, 0, UNIT.DEGREE);
        ...
        if (crs) {
            ...
            if (pointOrder !== undefined) {
                ...
                if (layer.type == 'poly') {
                    ...
                    if (features.length) {
                        result.feature = this.featureToolBox.GeoJSON2Box(features, pointOrder, layer.threejsLayer);
                    }
                }
