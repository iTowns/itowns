define(['THREE','./LRU', './PointCloudOctreeNode', './PointCloudMaterial', './PointCloudOctreeGeometryNode', './PointSizeType', './PointColorType', './BinaryHeap', './utils'], 
function(THREE, LRU, PointCloudOctreeNode, PointCloudMaterial, PointCloudOctreeGeometryNode, PointSizeType, PointColorType, BinaryHeap, utils){

var PointCloudOctree = function(geometry, material){
	THREE.Object3D.call( this );
	
        //must be carefully becaus Potree call directly Potree.PointCloudOctree
...
PointCloudOctree.prototype = Object.create(THREE.Object3D.prototype);

PointCloudOctree.prototype.updateVisibility = function(camera, renderer){

	this.numVisibleNodes = 0;
	this.numVisiblePoints = 0;
	
	// frustum in object space
      	//camera_o.updateMatrixWorld();
        //var camera = camera_o.clone();
	...
	while(priorityQueue.size() > 0){
		...
		if(node instanceof PointCloudOctreeGeometryNode){
		...
		if(node instanceof PointCloudOctreeNode){
			//PointCloudOctree.lru.touch(node.geometryNode);
...
PointCloudOctree.prototype.updateVisibleBounds = function(){
...
PointCloudOctree.prototype.updateMaterial = function(material, visibleNodes, camera, renderer){
	...
	if(material.pointSizeType){
		if(material.pointSizeType === PointSizeType.ADAPTIVE 
			|| material.pointColorType === PointColorType.OCTREE_DEPTH){
...
PointCloudOctree.prototype.update = function(camera, renderer){
	...
	this.updateVisibleBounds();
        ...
        //remove PointCloudOctree.lru.freeMemory by binding this
	this.lru.freeMemory();
...
PointCloudOctree.prototype.updateVisibilityTexture = function(material, visibleNodes){
	...
	for(var i = 0; i < visibleNodes.length; i++){
		...
		for(var j = 0; j < 8; j++){
			var child = node.children[j];
			if(child instanceof PointCloudOctreeNode && child.sceneNode.visible){
				children.push(child);
			}
		}
...
PointCloudOctree.prototype.nodesOnRay = function(nodes, ray){
	var nodesOnRay = [];
...
PointCloudOctree.prototype.updateMatrixWorld = function( force ){
...
PointCloudOctree.prototype.hideDescendants = function(object){
	var stack = [];
...
PointCloudOctree.prototype.moveToOrigin = function(){
    this.position.set(0,0,0);
    this.updateMatrixWorld(true);
    var box = this.boundingBox;
    var transform = this.matrixWorld;
    var tBox = utils.computeTransformedBoundingBox(box, transform);
    this.position.set(0,0,0).sub(tBox.center());
};

PointCloudOctree.prototype.moveToGroundPlane = function(){
    this.updateMatrixWorld(true);
    var box = this.boundingBox;
    var transform = this.matrixWorld;
    var tBox = utils.computeTransformedBoundingBox(box, transform);
...
PointCloudOctree.prototype.getBoundingBoxWorld = function(){
	this.updateMatrixWorld(true);
    var box = this.boundingBox;
    var transform = this.matrixWorld;
    var tBox = utils.computeTransformedBoundingBox(box, transform);
 ...
 *
 */
PointCloudOctree.prototype.getPointsInProfile = function(profile, maxDepth){
	var points = {
		segments: [],
 ...
 *
 */
PointCloudOctree.prototype.getProfile = function(start, end, width, depth, callback){
	if(callback !== undefined){
		this.profileRequests.push(new ProfileRequest(start, end, width, depth, callback));
	}else{
		var stack = [];
		stack.push(this);
...
PointCloudOctree.prototype.getVisibleExtent = function(){
 ...
 * 
 */
PointCloudOctree.prototype.pick = function(renderer, camera, ray, params){
	...
	if(!this.pickMaterial){
		this.pickMaterial = new PointCloudMaterial();
		this.pickMaterial.pointColorType = PointColorType.POINT_INDEX;
	}
...
PointCloudOctree.prototype.createDEM = function(node){	
...
PointCloudOctree.prototype.getDEMHeight = function(position){
...
PointCloudOctree.prototype.generateTerain = function(){
...
Object.defineProperty(PointCloudOctree.prototype, "progress", {
	get: function(){
