define(['THREE', './PointCloudArena4DGeometryNode', './PointCloudMaterial', './TreeType', './PointSizeType', './PointCloudArena4DProxyNode', './PointColorType'], 
function(THREE, PointCloudArena4DGeometryNode, PointCloudMaterial, TreeType, PointSizeType, PointCloudArena4DProxyNode, PointColorType){


var PointCloudArena4D = function(geometry){
...
PointCloudArena4D.prototype = Object.create(THREE.Object3D.prototype);

PointCloudArena4D.prototype.updateMaterial = function(camera, renderer){
...
PointCloudArena4D.prototype.hideDescendants = function(object){
	var stack = [];
...
PointCloudArena4D.prototype.updateMatrixWorld = function( force ){
...
PointCloudArena4D.prototype.update = function(camera, renderer){
	var geometry = this.pcoGeometry;

	if(!geometry.root){
		return;
	}else if(!this.rootProxyGenerated){
		var rootProxy = new PointCloudArena4DProxyNode(this.pcoGeometry.root);
		this.add(rootProxy);
	...
	while(stack.length > 0){
		...
		if (node instanceof PointCloudArena4DProxyNode) {
			...
			if(geometryNode.loaded === true){
				...
				this.loadQueue.push(element);
                        ...
                        //ATTENTION: this.visiblePointsTarget may have scope problem, ori: pointcloud.visiblePointsTarget
...
PointCloudArena4D.prototype.replaceProxy = function(proxy){
	...
	if(geometryNode.loaded === true){
		...
		//var current = parent;
		//while(!(current instanceof PointCloudArena4D)){
		...
		if(geometryNode.left){
			...
			var childProxy = new PointCloudArena4DProxyNode(child);
			node.add(childProxy);
		}
...
PointCloudArena4D.prototype.updateLoadQueue = function(vn){
...
PointCloudArena4D.prototype.getVisibleGeometry = function(camera){
	var visibleGeometry = [];
...
PointCloudArena4D.prototype.nodesOnRay = function(nodes, ray){
	var nodesOnRay = [];
...
PointCloudArena4D.prototype.pick = function(renderer, camera, ray, params){
	...
	if(!this.pickMaterial){
		this.pickMaterial = new PointCloudMaterial({treeType: TreeType.KDTREE});
		this.pickMaterial.pointColorType = PointColorType.POINT_INDEX;
		this.pickMaterial.pointSizeType = PointSizeType.FIXED;
	}
	...
	if(this.pickMaterial.pointSizeType === PointSizeType.ADAPTIVE){
...
PointCloudArena4D.prototype.updateVisibilityTexture = function(material, visibleNodes){
...
Object.defineProperty(PointCloudArena4D.prototype, "progress", {
	get: function(){
