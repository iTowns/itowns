...
export class LayeredMaterial extends THREE.ShaderMaterial {
    ...
    public updateLayersUniforms(): void {
        ...
        if ((this.elevationTileId !== undefined
            && this.getColorLayer(this.elevationTileId))
    ...
    public removeLayer(layerId: string): void {
        const index = this.colorTiles.findIndex(l => l.id === layerId);
        if (index > -1) {
            this.colorTiles[index].dispose();
            this.colorTiles.splice(index, 1);
            const idSeq = this.colorTileIds.indexOf(layerId);
            if (idSeq > -1) {
    ...
    public addColorLayer(rasterNode: RasterColorTile) {
        if (rasterNode.layer.id in this.colorTiles) {
            console.warn(
                'Layer "{layer.id}" already present in material, overwriting.',
            );
        }
        this.colorTiles.push(rasterNode);
    }

    public setElevationLayer(rasterNode: RasterElevationTile) {
        const old = this.elevationTile;
        if (old !== undefined) {
            old.dispose();
        }

        this.elevationTile = rasterNode;
    }

    public getColorLayer(id: string): RasterColorTile | undefined {
        return this.colorTiles.find(l => l.id === id);
    }

    public getColorLayers(ids: string[]): RasterColorTile[] {
        return this.colorTiles.filter(l => ids.includes(l.id));
    }

    public getElevationLayer(): RasterElevationTile | undefined {
