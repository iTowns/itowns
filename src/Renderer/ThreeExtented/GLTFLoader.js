...
THREE.GLTFLoader.prototype = {
	...
	parse: function( json, callback, path ) {
		...
		var glTFParser = new this.parser( json, {
			...
			binary: this.binary || null,
			binaryGltfArray: this.binaryGltfArray || null
		});
...
GLTFParser.prototype.parse = function() {
	...
	return this._withDependencies([
		...
		return {scene: scene, cameras: cameras, animations: animations};
	...
	return _each( this.json.shaders, function( shader/*, shaderId */) {
		if(this.options.binary){
			if(shader.extensions && shader.extensions.KHR_binary_glTF) {
				var bufferView = this.json.bufferViews[shader.extensions.KHR_binary_glTF.bufferView];
				var text = this.options.binaryGltfArray.subarray(bufferView.byteOffset, bufferView.byteOffset + bufferView.byteLength);
				return new Promise( function( resolve ) {
					resolve(decodeFromCharCode(text));
				}.bind( this ));
			}
			else
	...
	return this._withDependencies([
		...
		return _each( this.json.accessors, function( accessor/*, accessorId */) {
			...
			if (accessor.byteStride && accessor.byteStride !== itemBytes ) {
				...
				var array = new TypedArray( arraybuffer/*.buffer*/);
				...
				array = new TypedArray( arraybuffer/*.buffer*/, accessor.byteOffset, accessor.count * itemSize );
...
GLTFParser.prototype.loadCameras = function() {
	...
	return _each( this.json.cameras, function( camera/*, cameraId */) {
		...
		if ( camera.type == "perspective" && camera.perspective ) {
			...
			let _camera = new THREE.PerspectiveCamera( THREE.Math.radToDeg( xfov ), aspect_ratio, camera.perspective.znear || 1, camera.perspective.zfar || 2e6 );
			...
			let _camera = new THREE.OrthographicCamera( window.innerWidth / - 2, window.innerWidth / 2, window.innerHeight / 2, window.innerHeight / - 2, camera.orthographic.znear, camera.orthographic.zfar );
			_camera.name = camera.name;
