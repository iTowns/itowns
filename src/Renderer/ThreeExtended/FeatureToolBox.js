...
const geoCoord = new GeoCoordinate(0, 0, 0, UNIT.DEGREE);

FeatureToolBox.prototype.geoArrayTo3D = function geoArrayTo3D(ellipsoid, point, altitude, pointOrder) {
    return this.geoCoordTo3D(ellipsoid, parseFloat(point[pointOrder.long]), parseFloat(point[pointOrder.lat]), altitude);
};

FeatureToolBox.prototype.geoCoordTo3D = function geoCoordTo3D(ellipsoid, longitude, latitude, altitude) {
    geoCoord.set(longitude, latitude, altitude, UNIT.DEGREE);
    return ellipsoid.cartographicToCartesian(geoCoord);
};

FeatureToolBox.prototype.GeoJSON2Polygon = function GeoJSON2Polygon(ellipsoid, features, pointOrder) {
    ...
    for (let r = 0; r < features.length; r++) {
        ...
        if (polygon.length > 2 && altitude != 9999) {
            for (let j = 0; j < polygon.length; ++j) {
                const spt = this.geoArrayTo3D(ellipsoid, polygon[j], altitude, pointOrder);
...
FeatureToolBox.prototype.GeoJSON2Box = function GeoJSON2Box(ellipsoid, features, pointOrder) {
    ...
    const center = this.geoArrayTo3D(ellipsoid, features[0].geometry.coordinates[0][0][0], 0, pointOrder);
    ...
    for (let r = 0; r < features.length; r++) {
        ...
        if (polygon.length > 2) {
            ...
            for (let j = 0; j < polygon.length - 1; ++j) {
                const pgeo2 = this.geoArrayTo3D(ellipsoid, polygon[j], altitude, pointOrder).sub(center);
                const pgeo1 = this.geoArrayTo3D(ellipsoid, polygon[j], altitude - height, pointOrder).sub(center);
        ...
        for (let i = 0; i < triangles.length; i += 3) {
            roofGeometry.vertices.push(this.geoCoordTo3D(ellipsoid, arrPoint2D[triangles[i] * 2], arrPoint2D[triangles[i] * 2 + 1], altitude).sub(center));
            roofGeometry.vertices.push(this.geoCoordTo3D(ellipsoid, arrPoint2D[triangles[i + 1] * 2], arrPoint2D[triangles[i + 1] * 2 + 1], altitude).sub(center));
            roofGeometry.vertices.push(this.geoCoordTo3D(ellipsoid, arrPoint2D[triangles[i + 2] * 2], arrPoint2D[triangles[i + 2] * 2 + 1], altitude).sub(center));
 ...
 */
FeatureToolBox.prototype.GeoJSON2Line = function GeoJSON2Line(ellipsoid, features, box, layer, pointOrder) {
    ...
    for (let i = 0; i < features.length; i++) {
        ...
        if (coords.length) {
            if (i === 0) {
                center.copy(this.geoArrayTo3D(ellipsoid, coords[0], altitude, pointOrder));
            }
            ...
            for (let k = 0; k < coords.length; k++) {
                const pt = this.geoArrayTo3D(ellipsoid, coords[k], altitude, pointOrder).sub(center);
                line.addPoint(pt);
            }
 ...
 */
FeatureToolBox.prototype.GeoJSON2Point = function GeoJSON2Point(ellipsoid, features, bbox, layer, pointOrder) {
    ...
    for (let i = 0; i < features.length; i++) {
        ...
        const normalGlobe = ellipsoid.geodeticSurfaceNormalCartographic(geoCoord);
        const centerPoint = ellipsoid.cartographicToCartesian(geoCoord);
...
FeatureToolBox.prototype.processingGeoJSON = function processingGeoJSON(ellipsoid, json) {
    ...
    for (let nFeature = 0; nFeature < jsonFeatures.length; nFeature++) {
        ...
        if (jsonFeatures[nFeature].type == 'Point') {
            const vertex = this.geoArrayTo3D(ellipsoid, jsonFeatures[nFeature].coordinates, altitude, pointOrder);
            ...
            for (let point_num = 0; point_num < jsonFeatures[nFeature].coordinates.length; point_num++) {
                ...
                const vertex = this.geoArrayTo3D(ellipsoid, coordinate_array, altitude, pointOrder);
                geometry.vertices.push(vertex);
            }
            bpoint = true;
