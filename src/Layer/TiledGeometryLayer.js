...
const diagonalTilePx = Math.pow(2 * (SIZE_TEXTURE_TILE * SIZE_TEXTURE_TILE), 0.5);
...
class TiledGeometryLayer extends GeometryLayer {
    ...
    subdivision(context, layer, node) {
        ...
        node.px = context.camera._preSSE * (2 * node.boundingSphere.radius * subdivisionVector.x) / distance;

        // The screen space error is calculated to have a correct texture display.
        // For the projection of a texture's texel to be less than or equal to one pixel
        const sse = node.px / (diagonalTilePx * 2);
