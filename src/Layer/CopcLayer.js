...
class CopcLayer extends PointCloudLayer {
    ...
    constructor(id, config) {
        ...
        if (this.source.isSource) {
            this.source = {
                whenReady: [this.source],
            };
        }
        ...
        this.source.whenReady.forEach((source) => {
            promises.push(source.whenReady.then((/** @type {CopcSource} */ source) => {
    ...
    get spacing() {
        if (this.source.whenReady.constructor === Array) {
            const spacings = new Set(this.source.whenReady.map(source => source.info.spacing)).size;

            if (spacings > 1) {
                console.warn("Warning, multiSpacing isn't not handled yet");
            }

            return this.source.whenReady[0].info.spacing;
        } else {
