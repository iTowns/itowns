...
function computePitchs(textures, extentsDestination, nodeLayer) {
    return extentsDestination
        .map((ext, i) => (textures[i] ? ext.offsetToParent(textures[i].extent, nodeLayer.offsetScales[i]) : undefined));
}
...
export function updateLayeredMaterialNodeImagery(context, layer, node, parent) {
    ...
    return context.scheduler.execute(command).then(
        (results) => {
            ...
            const pitchs = computePitchs(results, extentsDestination, nodeLayer);
