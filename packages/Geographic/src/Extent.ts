...
class Extent {
    ...
    /**
     * Tests whether this extent intersects another extent,
     * excluding boundary-only contact.
     *
     ...
     *
     * Extents that only touch at an edge or a corner are treated
     * as no intersecting.
     *
    ...
    intersectsExtentExcludingBoundary(extent: Extent) {
        return Extent.intersectsExtentExcludingBoundary(this, extent);
    }
     ...
     * is reprojected into the CRS of `extentA` before performing the test.
     *
     * Extents that touch at an edge or a corner are treated as intersecting.
     *
     * @param extentA - The reference extent.
     * @param extentB - The extent to test against.
     *
    ...
    static intersectsExtentExcludingBoundary(extentA: Extent, extentB: Extent) {
        const other = extentB.crs == extentA.crs ? extentB : extentB.as(extentA.crs, _extent);
        return !(extentA.west > other.east ||
            extentA.east < other.west ||
            extentA.south > other.north ||
     ...
     *
     * If the extents do not intersect, an empty extent is returned.
     *
