...
class GeotiffNode {
    ...
    async extractTexture(
        ...
        const imageWindow = textureExtent && this.#extentToImageWindow(textureExtent);
...
async function parse(data: any, options: any) {
    ...
    const textureFoo = <TextureWithExtent> await geotiffNode.extractTexture({
        textureDimensions: tileRasterDimensions,
        resampleMethod,
        defaultAlpha,
        // TODO: find a way to add pool
    });
    textureFoo.extent = extent;
