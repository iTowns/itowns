...
abstract class PointCloudLayer<S extends PointCloudSource = PointCloudSource>
    ...
    setNodeVisibility(node: PointCloudNode, visible: boolean) {
        const { obj } = node;
        ...
        if (visible) {
            ...
            for (const child of node.children) {
                if (child.visible) {
                    this.setNodeVisibility(child, false);
                }
            }
        }
        node.visible = visible;
        this.dispatchEvent({ type: 'node-visibility-change', tile: node, visible });
    }
            ...
            if (!elt.obj && !elt.promise) {
                ...
                elt.promise = context.scheduler.execute({
                    ...
                    earlyDropFunction: cmd =>
                        cmd.requester.notVisibleSince !== undefined || !this.visible,
        ...
        while (stack.length > 0) {
            ...
            for (const child of node.children) {
                if (node.sse >= this.sseThreshold) {
                    stack.push({ node: child, weight: node.sse });
